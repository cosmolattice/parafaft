...
void run_benchmark(int N, int rank, int mpi_size, int iterations) {
  ...
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "Benchmark: ParaFaFT C2C (cuFFT) " << D << "D, N=" << N
...

  {
    ...
    std::vector<std::complex<double>> host_buffer(buffer_size);
  ...
  if (rank == 0) {
    ...
    write_csv_cuda("bench_c2c_cuda.csv", mpi_size, N, parafaft_mean,
                   parafaft_std, iterations);
    std::cout << "CSV written to bench_c2c_cuda.csv" << std::endl;
  }
}
