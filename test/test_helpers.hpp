...
//   - iterate_nd<D>         : c2c nested-loop replacement (runtime dims)
// ============================================================================
...
// --------------------------------------------------------------------------
template <int D>
std::vector<std::complex<double>> generate_gaussian_nd(int N, double sigma) {
  int total = 1;
  for (int d = 0; d < D; ++d)
    total *= N;
...
// --------------------------------------------------------------------------
template <int D>
std::vector<std::complex<double>> generate_random_poly_nd(int N) {
  int total = 1;
  for (int d = 0; d < D; ++d)
    total *= N;
...
// --------------------------------------------------------------------------
template <int D>
std::vector<std::complex<double>> generate_c2c_data_nd(int N, int shape_id) {
  switch (shape_id) {
  case SHAPE_GAUSSIAN:
...
// --------------------------------------------------------------------------
template <int D>
double point_value(const std::array<int, D> &gidx, int N, int shape_id) {
  const double center = N / 2.0;
  switch (shape_id) {
  case SHAPE_GAUSSIAN: {
...
// Uses the c2c fftw_plan_dft which accepts arbitrary rank.
// --------------------------------------------------------------------------
template <int D> void fftw_c2c_reference_nd(std::complex<double> *data, int N) {
  ...
  fftw_plan plan = fftw_plan_dft(
      D, n.data(), reinterpret_cast<fftw_complex *>(data),
...
inline void cufft_c2c_reference_2d(std::complex<double> *host_data, int N0,
                                   int N1) {
  const int total = N0 * N1;
  cufftDoubleComplex *d_data = nullptr;
  cudaMalloc((void **)&d_data, total * sizeof(cufftDoubleComplex));
  cudaMemcpy(d_data, host_data, total * sizeof(cufftDoubleComplex),
  ...
  cudaMemcpy(host_data, d_data, total * sizeof(cufftDoubleComplex),
             cudaMemcpyDeviceToHost);
  cudaFree(d_data);
}

inline void cufft_c2c_reference_3d(std::complex<double> *host_data, int N0,
                                   int N1, int N2) {
...
template <int D> int compare_cufft_vs_fftw(int N, int rank) {
  static_assert(D == 2 || D == 3,
                "cuFFT library reference only available for D=2,3");
  if (rank != 0)
    return 0;

  std::cout << "\n==========================================" << std::endl;
  std::cout << "Sanity check: Comparing (sequential) FFTW and cuFFT for " << D
  ...
  if (max_error < 1e-10) {
    std::cout << "Test passed: FFTW and cuFFT produce identical results."
              << std::endl;
    std::cout << "Maximum error: " << max_error << std::endl;
    return 0;
  } else {
    std::cout << "Test failed: FFTW and cuFFT produce different results."
              << std::endl;
    std::cout << "Maximum error: " << max_error << std::endl;
    return 1;
  }
...
inline void hipfft_c2c_reference_2d(std::complex<double> *host_data, int N0,
                                    int N1) {
  const int total = N0 * N1;
  hipfftDoubleComplex *d_data = nullptr;
  hipMalloc((void **)&d_data, total * sizeof(hipfftDoubleComplex));
  hipMemcpy(d_data, host_data, total * sizeof(hipfftDoubleComplex),
  ...
  hipMemcpy(host_data, d_data, total * sizeof(hipfftDoubleComplex),
            hipMemcpyDeviceToHost);
  hipFree(d_data);
}

inline void hipfft_c2c_reference_3d(std::complex<double> *host_data, int N0,
                                    int N1, int N2) {
  const int total = N0 * N1 * N2;
  hipfftDoubleComplex *d_data = nullptr;
  hipMalloc((void **)&d_data, total * sizeof(hipfftDoubleComplex));
...
template <int D> int compare_hipfft_vs_fftw(int N, int rank) {
  static_assert(D == 2 || D == 3,
                "hipFFT library reference only available for D=2,3");
  if (rank != 0)
    return 0;

  std::cout << "\n==========================================" << std::endl;
  std::cout << "Sanity check: Comparing (sequential) FFTW and hipFFT for " << D
            << "D C2C transform of size ";
  std::array<int, D> s;
  s.fill(N);
  print_shape<D>(std::cout, s.data());
  ...
  if (max_error < 1e-10) {
    std::cout << "Test passed: FFTW and hipFFT produce identical results."
              << std::endl;
    std::cout << "Maximum error: " << max_error << std::endl;
    return 0;
  } else {
    std::cout << "Test failed: FFTW and hipFFT produce different results."
              << std::endl;
    std::cout << "Maximum error: " << max_error << std::endl;
    return 1;
  }
...
// --------------------------------------------------------------------------
template <int D>
inline int nd_index_real(const std::array<int, D> &idx, const int shape[D]) {
  int flat = idx[0];
  for (int d = 1; d < D; ++d)
    flat = flat * shape[d] + idx[d];
  return flat;
...
// --------------------------------------------------------------------------
template <int D>
inline int nd_index_real_padded(const std::array<int, D> &idx,
                                const int shape[D]) {
...
// --------------------------------------------------------------------------
template <int D>
inline int nd_index_complex(const std::array<int, D> &idx,
                            const int complex_shape[D]) {
  int flat = idx[0];
  for (int d = 1; d < D; ++d)
...
// --------------------------------------------------------------------------
template <int D>
std::vector<double> generate_gaussian_r2c_nd(int N, double sigma) {
  // Total real elements
  int real_total = 1;
  for (int d = 0; d < D; ++d)
    real_total *= N;
...
// --------------------------------------------------------------------------
template <int D>
std::vector<double> generate_gaussian_r2c_padded_nd(int N, double sigma) {
  int total_size = 1;
  for (int d = 0; d < D - 1; ++d)
    total_size *= N;
  total_size *= (N + 2);
...
// --------------------------------------------------------------------------
template <int D>
std::vector<double> generate_r2c_padded_data_nd(int N, int shape_id) {
  switch (shape_id) {
  case SHAPE_GAUSSIAN:
...
template <int D> void fftw_r2c_reference_nd(double *host_data, int N) {
  ...
  for (int d = 0; d < D - 1; ++d)
    complex_doubles *= N;
  complex_doubles *=
      (N + 2); // (N/2+1) complex entries = (N+2) doubles in last dim
  ...
  fftw_plan plan = fftw_plan_dft_r2c(
      D, n.data(), temp.data(), reinterpret_cast<fftw_complex *>(host_data),
      FFTW_ESTIMATE);
  fftw_execute(plan);
  fftw_destroy_plan(plan);
}
...
template <int D> void fftw_r2c_reference_padded_nd(double *host_data, int N) {
  ...
  fftw_plan plan = fftw_plan_dft_r2c(
      D, n.data(), host_data, reinterpret_cast<fftw_complex *>(host_data),
      FFTW_ESTIMATE);
  fftw_execute(plan);
  fftw_destroy_plan(plan);
}
...
inline void cufft_r2c_reference_2d(double *host_data, int N0, int N1) {
  ...
  cudaMemcpy(d_real, host_data, real_size * sizeof(cufftDoubleReal),
             cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(host_data, d_complex, complex_size * sizeof(cufftDoubleComplex),
             cudaMemcpyDeviceToHost);
  cudaFree(d_real);
  cudaFree(d_complex);
}
...
inline void cufft_r2c_reference_padded_2d(double *host_data, int N0, int N1) {
  ...
  cudaMemcpy(d_data, host_data, total_size * sizeof(cufftDoubleComplex),
             cudaMemcpyHostToDevice);

  cufftHandle plan;
  cufftPlan2d(&plan, N0, N1, CUFFT_D2Z);
  cufftExecD2Z(plan, reinterpret_cast<cufftDoubleReal *>(d_data),
               reinterpret_cast<cufftDoubleComplex *>(d_data));
  cufftDestroy(plan);

  cudaMemcpy(host_data, d_data, total_size * sizeof(cufftDoubleComplex),
             cudaMemcpyDeviceToHost);
  cudaFree(d_data);
}

inline void cufft_r2c_reference_padded_3d(double *host_data, int N0, int N1,
                                          int N2) {
...
template <int D> int compare_cufft_vs_fftw_r2c(int N, int rank) {
  static_assert(D == 2 || D == 3,
                "cuFFT R2C library reference only available for D=2,3");
  if (rank != 0)
    return 0;
  ...
  std::complex<double> *fc =
      reinterpret_cast<std::complex<double> *>(fftw_ref.data());
  std::complex<double> *cc =
      reinterpret_cast<std::complex<double> *>(cufft_ref.data());
...
template <int D> int compare_cufft_vs_fftw_r2c_padded(int N, int rank) {
  static_assert(D == 2 || D == 3,
                "cuFFT R2C padded library reference only available for D=2,3");
  if (rank != 0)
    return 0;

  std::cout << "\n==========================================" << std::endl;
  std::cout << "Sanity check: Comparing (sequential) FFTW and cuFFT for " << D
            << "D padded R2C transform of size ";
  std::array<int, D> s;
  s.fill(N);
  print_shape<D>(std::cout, s.data());
  std::cout << std::endl;
...
inline void hipfft_r2c_reference_2d(double *host_data, int N0, int N1) {
  ...
  hipMemcpy(d_real, host_data, real_size * sizeof(hipfftDoubleReal),
            hipMemcpyHostToDevice);
  ...
  hipMemcpy(host_data, d_complex, complex_size * sizeof(hipfftDoubleComplex),
            hipMemcpyDeviceToHost);
  hipFree(d_real);
  hipFree(d_complex);
}
...
inline void hipfft_r2c_reference_padded_2d(double *host_data, int N0, int N1) {
  ...
  hipMemcpy(d_data, host_data, total_size * sizeof(hipfftDoubleComplex),
            hipMemcpyHostToDevice);

  hipfftHandle plan;
  hipfftPlan2d(&plan, N0, N1, HIPFFT_D2Z);
  hipfftExecD2Z(plan, reinterpret_cast<hipfftDoubleReal *>(d_data),
                reinterpret_cast<hipfftDoubleComplex *>(d_data));
  hipfftDestroy(plan);

  hipMemcpy(host_data, d_data, total_size * sizeof(hipfftDoubleComplex),
            hipMemcpyDeviceToHost);
  hipFree(d_data);
}

inline void hipfft_r2c_reference_padded_3d(double *host_data, int N0, int N1,
                                           int N2) {
  const int total_size = N0 * N1 * (N2 / 2 + 1);
  hipfftDoubleComplex *d_data = nullptr;
  hipMalloc((void **)&d_data, total_size * sizeof(hipfftDoubleComplex));
...
template <int D> int compare_hipfft_vs_fftw_r2c(int N, int rank) {
  static_assert(D == 2 || D == 3,
                "hipFFT R2C library reference only available for D=2,3");
  if (rank != 0)
    return 0;
  ...
  std::complex<double> *hc =
      reinterpret_cast<std::complex<double> *>(hipfft_ref.data());
  for (int i = 0; i < complex_total; ++i) {
...
template <int D> int compare_hipfft_vs_fftw_r2c_padded(int N, int rank) {
  static_assert(D == 2 || D == 3,
                "hipFFT R2C padded library reference only available for D=2,3");
  if (rank != 0)
    return 0;
