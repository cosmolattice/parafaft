...
int main(int argc, char **argv)
{
  ...
  if (rank == 0) {
    std::cout << "R2C FFT Test (cuFFT backend): " << N0 << "x" << N1 << "x" << N2 << " with " << size << " processes\n";
  ...
  MPI_Gatherv(complex_data.data(), local_complex_size, MPI_C_DOUBLE_COMPLEX, gathered_data.data(), sizes.data(),
              displs.data(), MPI_C_DOUBLE_COMPLEX, 0, MPI_COMM_WORLD);

  // On rank 0: reconstruct global array
  int test_passed = 1;
  if (rank == 0) {
