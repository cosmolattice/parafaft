...
// Run a roundtrip test for a given grid size N, returning max error.
// Returns 0 on success, 1 on failure.
int run_roundtrip(int N, int rank, int size) {
    ...
    // Create R2C FFT object with Alltoall exchange method
    parafaft::ParaFaFT_R2C<3, parafaft::FFTWBackend, parafaft::ExchangeMethod::Alltoall> fft(global_shape);
    ...
    if (rank == 0) {
        std::cout << "R2C Alltoall Roundtrip Test: " << N0 << "x" << N1 << "x" << N2
                  << " with " << size << " processes\n";
    }
...
int main(int argc, char** argv) {
    ...
    // Test 1: Power-of-two size (N=32)
    failures += run_roundtrip(32, rank, size);

    // Test 2: Non-power-of-two size (N=30) to exercise padding logic
    // 30/2+1=16 doesn't divide evenly by P=4 in all grid configurations
    failures += run_roundtrip(30, rank, size);

    if (rank == 0) {
        if (failures == 0) {
            std::cout << "All Alltoall roundtrip tests PASSED\n";
        } else {
            std::cout << failures << " Alltoall roundtrip test(s) FAILED\n";
        }
    }
