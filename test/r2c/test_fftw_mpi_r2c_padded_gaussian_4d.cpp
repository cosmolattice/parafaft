// FFTW version - 4D padded gaussian test
...
std::vector<double> generate_padded_gaussian(int N0, int N1, int N2, int N3, double center0, double center1,
                                             double center2, double center3, double sigma)
{
  const int total_size = N0 * N1 * N2 * (N3 + 2);
    ...
    for (int i1 = 0; i1 < N1; ++i1) {
      for (int i2 = 0; i2 < N2; ++i2) {
        for (int i3 = 0; i3 < N3; ++i3) {
          ...
          int idx = ((i0 * N1 + i1) * N2 + i2) * (N3 + 2) + i3;
          data[idx] = value;
        }
      }
    }
...
void generate_fftw_r2c_reference(double *host_data, const int N0, const int N1, const int N2, const int N3)
{
  ...
  fftw_plan plan_forward =
      fftw_plan_dft_r2c(4, n, host_data, reinterpret_cast<fftw_complex *>(host_data), FFTW_ESTIMATE);
...
int compare_fftwBackend(const int N, int rank)
{
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "Testing parafaft fftwBackend for 4D R2C transform of size " << N << "x" << N << "x" << N << "x" << N
              << std::endl;
  }

  // Create global base data
  const auto global_original_data = generate_padded_gaussian(N, N, N, N, N / 2.0, N / 2.0, N / 2.0, N / 2.0, 4.0);
  ...
  parafaft::ParaFaFT_R2C<4, parafaft::FFTWBackend> fft(global_shape);
  ...
  const int padded_stride = N + 2; // Padded global row length
    ...
    for (int i1 = 0; i1 < local_real_shape[1]; ++i1) {
      for (int i2 = 0; i2 < local_real_shape[2]; ++i2) {
        for (int i3 = 0; i3 < local_real_shape[3]; ++i3) {
          int global_idx =
              (((real_start[0] + i0) * N + (real_start[1] + i1)) * N + (real_start[2] + i2)) * padded_stride +
              (real_start[3] + i3);
          int local_idx = ((i0 * local_real_shape[1] + i1) * local_real_shape[2] + i2) * padded_stride + i3;
...
int main(int argc, char **argv)
{
  ...
  if (rank == 0) {
    std::cout << "########################################" << std::endl;
    std::cout << "# TEST: r2c/fftw_mpi_r2c_padded_gaussian_4d" << std::endl;
    std::cout << "########################################" << std::endl;
  }
  ...
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "SUMMARY: r2c/fftw_mpi_r2c_padded_gaussian_4d" << std::endl;
