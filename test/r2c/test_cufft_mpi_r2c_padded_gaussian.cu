...
std::vector<double> generate_padded_gaussian(int N0, int N1, int N2, double center0, double center1, double center2,
                                             double sigma)
{
  const int total_size = N0 * N1 * (N2 + 2);
...
void generate_fftw_r2c_reference(double *host_data, const int N0, const int N1, const int N2)
{
  ...
  fftw_plan plan_forward =
      fftw_plan_dft_r2c_3d(N0, N1, N2, host_data, reinterpret_cast<fftw_complex *>(host_data), FFTW_ESTIMATE);
...
int compare_libraries(int N, int rank)
{
  ...
  auto fftw_reference = generate_padded_gaussian(N0, N1, N2, center0, center1, center2, sigma);
  auto cufft_reference = generate_padded_gaussian(N0, N1, N2, center0, center1, center2, sigma);
...
int compare_cuFFTBackend(const int N, int rank)
{
  ...
  const auto global_original_data = generate_padded_gaussian(N, N, N, N / 2.0, N / 2.0, N / 2.0, 4.0);
  ...
  const int local_padded_stride = local_real_shape[2] + 2;
  ...
  for (int i0 = 0; i0 < local_real_shape[0]; ++i0) {
    for (int i1 = 0; i1 < local_real_shape[1]; ++i1) {
      for (int i2 = 0; i2 < local_real_shape[2]; ++i2) {
        int global_idx = ((real_start[0] + i0) * N * global_padded_stride) +
                         ((real_start[1] + i1) * global_padded_stride) + (real_start[2] + i2);
        int local_idx = ((i0 * local_real_shape[1]) + i1) * local_padded_stride + i2;
...
int main(int argc, char **argv)
{
  ...
  if (rank == 0) {
    std::cout << "########################################" << std::endl;
    std::cout << "# TEST: r2c/cufft_mpi_r2c_padded_gaussian" << std::endl;
    std::cout << "########################################" << std::endl;
  }
  ...
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "SUMMARY: r2c/cufft_mpi_r2c_padded_gaussian" << std::endl;
