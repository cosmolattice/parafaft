// ============================================================================
// Generalized N-dimensional padded R2C Gaussian test for parafaft (cuFFT backend).
//
// Usage:  mpirun -np <P> ./test_cufft_mpi_r2c_padded_gaussian_nd [N] [D]
//
...
// transform with parafaft (MPI-distributed, cuFFT backend), and compares
// against a serial FFTW reference.
//
// For D = 2 or 3 an additional sanity check compares sequential cuFFT against
// sequential FFTW for the padded R2C transform.
// ============================================================================
...
template <int D> int compare_cuFFTBackend(const int N, int rank)
{
  ...
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "Testing parafaft CuFFTBackend for " << D << "D padded R2C transform of size ";
    std::array<int, D> s;
    s.fill(N);
  ...
  const auto global_original_data = generate_gaussian_r2c_padded_nd<D>(N, 4.0);
  ...
  // ---- Copy to device and perform in-place forward FFT ---------------------
...
int dispatch(int D, int N, int rank)
{
  ...
  // cuFFT-vs-FFTW padded R2C sanity check (only for D = 2, 3)
  if (D == 2)
    failures += compare_cufft_vs_fftw_r2c_padded<2>(N, rank);
  else if (D == 3)
    failures += compare_cufft_vs_fftw_r2c_padded<3>(N, rank);
...
int main(int argc, char **argv)
{
  ...
  if (rank == 0) {
    std::cout << "########################################" << std::endl;
    std::cout << "# TEST: r2c/cufft_mpi_r2c_padded_gaussian_nd" << std::endl;
