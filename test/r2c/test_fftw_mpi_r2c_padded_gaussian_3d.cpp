// FFTW version of test_mpi_gaussian.cpp
...
int compare_fftwBackend(const int N, int rank)
{
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "Testing parafaft fftwBackend for 3D R2C transform of size " << N << "x" << N << "x" << N << std::endl;
  }
  ...
  parafaft::ParaFaFT_R2C<3, parafaft::FFTWBackend> fft(global_shape);
  ...
  for (int i0 = 0; i0 < local_real_shape[0]; ++i0) {
    for (int i1 = 0; i1 < local_real_shape[1]; ++i1) {
      for (int i2 = 0; i2 < local_real_shape[2]; ++i2) {
        int global_idx =
            ((real_start[0] + i0) * N * padded_stride) + ((real_start[1] + i1) * padded_stride) + (real_start[2] + i2);
        int local_idx = ((i0 * local_real_shape[1]) + i1) * padded_stride + i2;
...
int main(int argc, char **argv)
{
  ...
  if (rank == 0) {
    std::cout << "########################################" << std::endl;
    std::cout << "# TEST: r2c/fftw_mpi_r2c_padded_gaussian" << std::endl;
    std::cout << "########################################" << std::endl;
  }
  ...
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "SUMMARY: r2c/fftw_mpi_r2c_padded_gaussian" << std::endl;
