// cuFFT version - 4D padded gaussian test
...
int compare_cuFFTBackend(const int N, int rank)
{
  ...
  const int local_padded_stride = local_real_shape[3] + 2;
    ...
    for (int i1 = 0; i1 < local_real_shape[1]; ++i1) {
      for (int i2 = 0; i2 < local_real_shape[2]; ++i2) {
        for (int i3 = 0; i3 < local_real_shape[3]; ++i3) {
          int global_idx =
              (((real_start[0] + i0) * N + (real_start[1] + i1)) * N + (real_start[2] + i2)) * global_padded_stride +
              (real_start[3] + i3);
          int local_idx = ((i0 * local_real_shape[1] + i1) * local_real_shape[2] + i2) * local_padded_stride + i3;
...
int main(int argc, char **argv)
{
  ...
  if (rank == 0) {
    std::cout << "########################################" << std::endl;
    std::cout << "# TEST: r2c/cufft_mpi_r2c_padded_gaussian_4d" << std::endl;
    std::cout << "########################################" << std::endl;
  }
  ...
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "SUMMARY: r2c/cufft_mpi_r2c_padded_gaussian_4d" << std::endl;
