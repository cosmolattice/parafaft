// FFTW version - 2D padded gaussian test
...
int compare_fftwBackend(const int N, int rank)
{
  ...
  for (int i0 = 0; i0 < local_real_shape[0]; ++i0) {
    for (int i1 = 0; i1 < local_real_shape[1]; ++i1) {
      int global_idx = (real_start[0] + i0) * padded_stride + (real_start[1] + i1);
      int local_idx = i0 * padded_stride + i1;
...
int main(int argc, char **argv)
{
  ...
  if (rank == 0) {
    std::cout << "########################################" << std::endl;
    std::cout << "# TEST: r2c/fftw_mpi_r2c_padded_gaussian_2d" << std::endl;
    std::cout << "########################################" << std::endl;
  }
  ...
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "SUMMARY: r2c/fftw_mpi_r2c_padded_gaussian_2d" << std::endl;
