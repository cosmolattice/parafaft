...
// Test: Compare C2C transform of a 2D Gaussian against raw FFTW library
...
std::vector<std::complex<double>> generate_gaussian(int N0, int N1, double center0, double center1, double sigma)
{
  const int total_size = N0 * N1;
...
void generate_fftw_c2c_reference(parafaft::FFTWBackend::Complex *host_data, const int N0, const int N1)
{
  // create fftw plan for 2d forward transform
  fftw_plan plan_forward = fftw_plan_dft_2d(N0, N1, reinterpret_cast<fftw_complex *>(host_data),
...
void generate_cuFFT_c2c_reference(parafaft::CuFFTBackend::Complex *host_data, const int N0, const int N1)
{
  const int total_size = N0 * N1;
...
int compare_libraries(int N, int rank)
{
  ...
  std::cout << "\n==========================================" << std::endl;
  std::cout << "Sanity check: Comparing (sequential) FFTW and cuFFT for 2D C2C transform of size " << N << "x" << N
            << std::endl;
  ...
  auto fftw_reference = generate_gaussian(N0, N1, center0, center1, sigma);
  auto cufft_reference = generate_gaussian(N0, N1, center0, center1, sigma);

  // Generate reference results
  generate_fftw_c2c_reference(fftw_reference.data(), N0, N1);
  generate_cuFFT_c2c_reference(reinterpret_cast<parafaft::CuFFTBackend::Complex *>(cufft_reference.data()), N0, N1);
...
int compare_cuFFTBackend(const int N, int rank)
{
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "Testing parafaft CuFFTBackend for 2D C2C transform of size " << N << "x" << N << std::endl;
  }
  ...
  generate_fftw_c2c_reference(global_fftw_reference.data(), N, N);

  // Create ParaFaFT object with cuFFT backend
  const int global_shape[2] = {N, N};
  parafaft::ParaFaFT<2, parafaft::CuFFTBackend> fft(global_shape);
  ...
  for (int i0 = 0; i0 < local_shape[0]; ++i0) {
    for (int i1 = 0; i1 < local_shape[1]; ++i1) {
      int global_idx = (global_start[0] + i0) * N + (global_start[1] + i1);
      int local_idx = i0 * local_shape[1] + i1;
      local_data[local_idx] = global_original_data[global_idx];
    }
  }
  std::cout << "Local shape on rank " << rank << ": " << local_shape[0] << " x " << local_shape[1] << std::endl;
  ...
  for (int i0 = 0; i0 < final_shape[0]; ++i0) {
    for (int i1 = 0; i1 < final_shape[1]; ++i1) {
      int global_idx = (final_start[0] + i0) * N + (final_start[1] + i1);
      int local_idx = i0 * final_shape[1] + i1;
...
int main(int argc, char **argv)
{
  ...
  if (rank == 0) {
    std::cout << "########################################" << std::endl;
    std::cout << "# TEST: c2c/cufft_mpi_c2c_gaussian_2d" << std::endl;
    std::cout << "########################################" << std::endl;
  }
