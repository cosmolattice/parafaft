...
// Test: Compare C2C transform of a 4D Gaussian against raw FFTW library
...
std::vector<std::complex<double>> generate_gaussian(int N0, int N1, int N2, int N3, double center0, double center1,
                                                    double center2, double center3, double sigma)
{
  const int total_size = N0 * N1 * N2 * N3;
                                 ...
                                 const int N3)
{
  ...
  fftw_plan plan_forward = fftw_plan_dft(4, n, reinterpret_cast<fftw_complex *>(host_data),
...
int compare_cuFFTBackend(const int N, int rank)
{
  if (rank == 0) {
    std::cout << "\n==========================================" << std::endl;
    std::cout << "Testing parafaft CuFFTBackend for 4D C2C transform of size " << N << "x" << N << "x" << N << "x" << N
              << std::endl;
  }
  ...
  generate_fftw_c2c_reference(global_fftw_reference.data(), N, N, N, N);

  // Create ParaFaFT object with cuFFT backend
  const int global_shape[4] = {N, N, N, N};
  parafaft::ParaFaFT<4, parafaft::CuFFTBackend> fft(global_shape);
  ...
  for (int i0 = 0; i0 < local_shape[0]; ++i0) {
    for (int i1 = 0; i1 < local_shape[1]; ++i1) {
      for (int i2 = 0; i2 < local_shape[2]; ++i2) {
        for (int i3 = 0; i3 < local_shape[3]; ++i3) {
          int global_idx = (((global_start[0] + i0) * N + (global_start[1] + i1)) * N + (global_start[2] + i2)) * N +
                           (global_start[3] + i3);
          int local_idx = ((i0 * local_shape[1] + i1) * local_shape[2] + i2) * local_shape[3] + i3;
  ...
  std::cout << "Local shape on rank " << rank << ": " << local_shape[0] << " x " << local_shape[1] << " x "
            << local_shape[2] << " x " << local_shape[3] << std::endl;
  ...
  for (int i0 = 0; i0 < final_shape[0]; ++i0) {
    for (int i1 = 0; i1 < final_shape[1]; ++i1) {
      for (int i2 = 0; i2 < final_shape[2]; ++i2) {
        for (int i3 = 0; i3 < final_shape[3]; ++i3) {
          int global_idx = (((final_start[0] + i0) * N + (final_start[1] + i1)) * N + (final_start[2] + i2)) * N +
                           (final_start[3] + i3);
          int local_idx = ((i0 * final_shape[1] + i1) * final_shape[2] + i2) * final_shape[3] + i3;
...
int main(int argc, char **argv)
{
  ...
  if (rank == 0) {
    std::cout << "########################################" << std::endl;
    std::cout << "# TEST: c2c/cufft_mpi_c2c_gaussian_4d" << std::endl;
    std::cout << "########################################" << std::endl;
  }
