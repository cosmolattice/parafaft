...
public:
    ...
    CuFFTBackend(CuFFTBackend&& other) noexcept
        : num_stages_(other.num_stages_),
          ...
          r2c_length_(other.r2c_length_),
          r2c_batch_(other.r2c_batch_),
          r2c_dist_(other.r2c_dist_),
          c2c_device_buffer_(other.c2c_device_buffer_),
    ...
    void create_stage_plan(
        ...
        if (c2c_device_buffer_ == nullptr || size_complex > c2c_buffer_size_) {
            ...
            check_cuda(cudaMalloc(&c2c_device_buffer_, size_complex * sizeof(cufftDoubleComplex)),
                       "cudaMalloc C2C buffer");
        }
        ...
        // Create forward plan
        check_cufft(cufftPlanMany(&forward_plans_[stage],
                                  1, n,
                                  embed, stride, dist,
                                  embed, stride, dist,
                                  CUFFT_Z2Z, batch),
        ...
        check_cufft(cufftPlanMany(&backward_plans_[stage],
                                  1, n,
                                  embed, stride, dist,
                                  embed, stride, dist,
                                  CUFFT_Z2Z, batch),
    ...
    void execute_stage(int stage, FFTDirection direction, std::complex<double>* data) {
        ...
        check_cuda(cudaMemcpy(c2c_device_buffer_, data, size_bytes,
        ...
        int cufft_direction = (direction == FFTDirection::Forward)
                              ? CUFFT_FORWARD
                              : CUFFT_INVERSE;

        check_cufft(cufftExecZ2Z(plan,
                                 static_cast<cufftDoubleComplex*>(c2c_device_buffer_),
                                 static_cast<cufftDoubleComplex*>(c2c_device_buffer_),
                                 cufft_direction),
                    "cufftExecZ2Z");
        ...
        check_cuda(cudaMemcpy(data, c2c_device_buffer_, size_bytes,
    ...
    void create_r2c_inplace_plan(
        ...
        if (device_buffer_ == nullptr || size_doubles > buffer_size_) {
            ...
            check_cuda(cudaMalloc(&device_buffer_, size_doubles * sizeof(double)),
                       "cudaMalloc R2C buffer");
        }
        ...
        check_cufft(cufftPlanMany(&r2c_plan_,
                                  1, n,
                                  inembed, stride, dist,           // Real input layout
    ...
    void execute_r2c_inplace(double* host_padded_real) {
        ...
        // Transfer host -> device
        check_cuda(cudaMemcpy(device_buffer_, host_padded_real, size_bytes,
                              cudaMemcpyHostToDevice),
                   "cudaMemcpy H2D R2C");

        // Execute R2C (D2Z = double to Z complex)
        // In-place: output overwrites input buffer
        check_cufft(cufftExecD2Z(r2c_plan_,
        ...
        check_cuda(cudaMemcpy(host_padded_real, device_buffer_, size_bytes,
                              cudaMemcpyDeviceToHost),
                   "cudaMemcpy D2H R2C");
    }
    ...
    void create_c2r_inplace_plan(
        ...
        check_cufft(cufftPlanMany(&c2r_plan_,
                                  1, n,
                                  inembed, stride, dist / 2,       // Complex input layout
    ...
    void execute_c2r_inplace(double* host_padded_real) {
        ...
        check_cufft(cufftExecZ2D(c2r_plan_,
                                 static_cast<cufftDoubleComplex*>(device_buffer_),
                                 static_cast<cufftDoubleReal*>(device_buffer_)),
                    "cufftExecZ2D");
