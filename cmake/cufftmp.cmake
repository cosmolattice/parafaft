...
#   2. NVHPC_ROOT / NVHPC env vars (module-set on HPC clusters)
#   3. HPC SDK math_libs path (auto-detected from standard paths)
#
# Outputs:
#   CUFFTMP_FOUND        - TRUE if cuFFTMp and NVSHMEM were found
#   CUFFTMP_INCLUDE_DIR  - cuFFTMp header directory
#   CUFFTMP_LIBRARY      - path to libcufftMp.so (non-NVHPC only)
#   NVSHMEM_INCLUDE_DIR  - NVSHMEM header directory
#   NVSHMEM_LIBRARY      - path to libnvshmem_host.so (non-NVHPC only)
#
# Under NVHPC, only the include directories are required; libraries are linked
# via `-cudalib=cufftmp`. Under other compilers (e.g. clang) the library paths
# are required and imported targets cufftMp::cufftMp / nvshmem::host are created.
...
# Honour module-set NVHPC root vars (common on HPC clusters)
foreach(_root_var NVHPC_ROOT NVHPC)
  if(DEFINED ENV{${_root_var}})
    set(_nvhpc_root "$ENV{${_root_var}}")
    if(IS_DIRECTORY "${_nvhpc_root}/math_libs")
      list(APPEND _cufftmp_search_paths "${_nvhpc_root}/math_libs")
    endif()
    if(IS_DIRECTORY "${_nvhpc_root}/comm_libs/nvshmem")
      list(APPEND _nvshmem_search_paths "${_nvhpc_root}/comm_libs/nvshmem")
    endif()
    # Some HPC SDK layouts nest the version dir inside math_libs
    file(GLOB _mlib_versions "${_nvhpc_root}/math_libs/*")
    foreach(_mv ${_mlib_versions})
      if(IS_DIRECTORY "${_mv}")
        list(APPEND _cufftmp_search_paths "${_mv}")
      endif()
    endforeach()
  endif()
endforeach()

# Auto-detect HPC SDK paths under /opt
  ...
  if(IS_DIRECTORY "${_base}")
    ...
    foreach(_ver ${_versions})
      if(IS_DIRECTORY "${_ver}/math_libs")
        ...
        file(GLOB _mlib_versions "${_ver}/math_libs/*")
        foreach(_mv ${_mlib_versions})
          if(IS_DIRECTORY "${_mv}")
            list(APPEND _cufftmp_search_paths "${_mv}")
          endif()
...
# Find cuFFTMp header (needed for both NVHPC and generic)
...
# Find NVSHMEM header (needed for both NVHPC and generic)
...
if(CMAKE_CXX_COMPILER_ID MATCHES "NVHPC")
  # NVHPC links cuFFTMp / NVSHMEM via -cudalib=cufftmp; only includes required.
  find_package_handle_standard_args(CUFFTMP
    REQUIRED_VARS CUFFTMP_INCLUDE_DIR NVSHMEM_INCLUDE_DIR
    FAIL_MESSAGE "cuFFTMp headers not found. Set CUFFTMP_HOME/NVSHMEM_HOME or ensure NVHPC_ROOT is exported."
  )
  mark_as_advanced(CUFFTMP_INCLUDE_DIR NVSHMEM_INCLUDE_DIR)
else()
